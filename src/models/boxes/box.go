...
func BoxInfoMenu(boxInfo machines.InfoResponse, machineHandle *machines.Handle, avatarUrl string) {
	...
	boxStatus := lipgloss.NewStyle().Foreground(format.Pink).Render(strings.ReplaceAll(boxInfoData.InfoStatus, "\n", " "))
	ip := boxInfoData.Ip
	var FormInfo = lipgloss.NewStyle().Background(format.BaseBG).Render(fmt.Sprintf(
		"%s \nIP: %s \nOS: %s\nDifficulty: %s\n", // move breach info to bottom
	...
	huh.NewForm(
		huh.NewGroup(
			...
			huh.NewSelect[string]().Title(lipgloss.NewStyle().Foreground(format.TextTitle).Background(format.BaseBG).Render("Box Action")).
                ...
                huh.NewNote().
                        Description(boxStatus).
                        Height(1),
			),

		// huh.NewNote().Title("").Description(boxStatus),
	).Run()
	switch boxAction {
	default:
		return
