...
type listData struct {
	Sherlocks     []sherlocks.SherlockItem
}

func getSherlocks(HTBClient *HTB.Client) (List listData) {
	sherlocks, err := HTBClient.Sherlocks.List().AllResults(ctx)
	if err != nil {
		fmt.Println(err.Error())
		os.Exit(1)
	}

	List.Sherlocks = sherlocks.Data

	return List
}

func NewModel(Boxes listData) model {
	columns := []table.Column{
		...
		table.NewFlexColumn(ColumnStatus, lipgloss.NewStyle().Foreground(format.TextTitle).Render("Sherlock Status"), 1).WithFiltered(true),
		table.NewFlexColumn(ColumnBoxID, lipgloss.NewStyle().Foreground(format.TextTitle).Render("Sherlock ID"), 1).WithFiltered(true),
	}
...
func genRows(boxes listData) (rows []table.Row) {
	for _, box := range boxes.Sherlocks {
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "enter", "return":
			Selected = m.Boxes.HighlightedRow().Data[ColumnBoxID].(string)
			cmds = append(cmds, tea.Quit)
		case "esc":
...
func Run(HTBClient *HTB.Client) string {
	Selected = ""
	fmt.Println(`fetching sherlocks.... If loaded before type "q" to skip loading`)
	// boxes := getBoxes(HTBClient)
	task := format.Task(func(a any) any {
		if client, ok := a.(*HTB.Client); ok {
			boxes := getSherlocks(client)
			return boxes
		}
		return nil
	})
	...
	boxes, ok := format.TaskResult.(listData)
	if !ok {
		panic("error checking task result for sherlocks!")
	}
